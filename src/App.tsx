...
export default function App() {
  ...
  useEffect(() => {
    if (authUser !== null || isGuest) initializeGame();
  }, [authUser, isGuest]);

  useEffect(() => {
    if (!gameEnded || !authUser || !token) return;
    fetch('/api/scores', {
      method: 'POST',
      headers: {
  ...
  }, [gameEnded, score, authUser, token]);
  ...
  const openBox = (boxId: number) => {
    ...
    setBoxes(prevBoxes => {
      ...
      if (updatedBoxes.some(box => box.isOpen && box.hasTreasure)) {
        setGameEnded(true);
      }
  ...
  const handleAuthSuccess = (user: AuthUser, authToken: string) => {
    localStorage.setItem('auth', JSON.stringify({ user, token: authToken }));
    setAuthUser(user);
    setToken(authToken);
  };

  if (!authUser && !isGuest) {
    return <AuthScreen onSuccess={handleAuthSuccess} onGuest={() => setIsGuest(true)} />;
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
        {boxes.map((box) => (
          <motion.div
            ...
            style={{ cursor: `url(${keyImage}) 8 24, pointer` }}
      ...
      {gameEnded && (
        ...
        >
          <div className="mb-4 p-6 bg-amber-200/80 backdrop-blur-sm rounded-xl shadow-lg border-2 border-amber-400">
            ...
            {scoreSaved && (
              <p className="text-sm text-green-600 mt-2 font-medium">Score saved!</p>
            )}
            {isGuest && (
              <p className="text-sm text-amber-500 mt-2">Sign in to save your scores</p>
            )}
          </div>
